...
###########################
# Data generation for stationary data
###########################
...
###########################
# Data generation for nonstationary data
###########################
# Params: n = number of training samples, p = number of features, n_test = number of test samples, alpha = autocorrelation
# Output: x = training data matrix, y = training outcome vector, x_test = test data matrix, y_test = test outcome vector 
data_gen_nonstationary <- function(n, p, n_test, alpha = 0){
  beta = rep(0, p)
  beta[1:4] = 1
  x = matrix(rnorm(n * p), nrow = n, ncol = p)
  x_test = matrix(rnorm(n_test * p), nrow = n_test, ncol = p)
  noise = arima.sim(list(order = c(1,1,0), ar = 0.99), n = n + n_test)/10
  y = x %*% beta + noise[1:n] + rnorm(n, mean = 0, sd = seq(n)**2)
  y_test = x_test %*% beta + noise[(n+1):(n+n_test)]
  return(list(x = x, y = y, x_test = x_test, y_test = y_test))
}
...
###########################
# Adaptive quantile-based forward cross-validation (AQFCV)
###########################
# Params: n = number of samples, fold_start = track number of folds passed, x = data matrix, y = outcome vector, w = window size, w_train = window size for training, w_val = window size for validation, lag = FCV lag, theta = AQFCV parameter inherited from the previous iteration
...
fcv_cal <- function(n, fold_start, x, y, w, w_train, w_val, lag, theta, err_val_list, err_test_list){
  w_test = w - w_train - w_val 
  start = 1 + lag * fold_start
  end = w + lag * fold_start
  nfold = floor((n-w)/lag) + 1
  for(fold in (fold_start+1):nfold){
    train_end = start + w_train - 1
    val_start = start + w_train 
    val_end = start + w_train + w_val - 1
    test_start = start + w_train + w_val
    err_val_list = c(err_val_list, mean(fit(x[start:train_end,], y[start:train_end], x[val_start:val_end,], y[val_start:val_end])))
    err_test_list = c(err_test_list, mean(fit(x[(val_end - w_train + 1):val_end,], y[(val_end - w_train + 1):val_end], x[test_start:end,], y[test_start:end])))
    start = start + lag
    end = end + lag 
  }
  ...
  rqdata <- data.frame(err_test = err_test_list, err_val = err_val_list)
  rqfit_lo <- rq(err_test ~ err_val, tau = 0.05 - min(theta, 0.0499), data = rqdata)
  err_hat_lo = predict(rqfit_lo, newdata = data.frame(err_val = c(point_val)))
  rqfit_hi <- rq(err_test ~ err_val, tau = 0.95 + min(theta, 0.0499), data = rqdata)
  err_hat_hi = predict(rqfit_hi, newdata = data.frame(err_val = c(point_val)))
  
  return(list(err_val_list = err_val_list, err_test_list = err_test_list, 
