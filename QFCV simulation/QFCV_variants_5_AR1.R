##############################################
# Compare different variants of QFCV methods for ARMA(1,0) noise and n_test = 5
##############################################
library(ggplot2)
library(ggpubr)
source("helper.R")
...
###########################
# Run simulation for n_test = 5 and noise is ARMA(1,0)
###########################
nsim = 500
n = 2000
n_test = 5
...
###########################
# Plot for n_test = 5 and noise is ARMA(1,0)
###########################
alpha_list = c(0.1,0.3,0.5,0.7,0.9)
coverage_cal = c()
length_cal = c()
...
for(alpha in alpha_list){
  load(paste0("sparse_linear_5_AR1_alpha-", alpha,".RData"))
