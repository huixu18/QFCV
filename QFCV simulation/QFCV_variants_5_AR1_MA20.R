##############################################
# Compare different variants of QFCV methods for ARMA(1,20) noise and n_test = 5
##############################################
library(ggplot2)
library(ggpubr)
source("helper.R")
...
for(alpha in alpha_list){
  ...
  save(out, file = paste0("sparse_linear_5_AR1_MA20_alpha-", alpha,".RData"))
  print(paste0("Results for alpha-", alpha, " saved to disk."))
}

###########################
# Plot for n_test = 5 and noise is ARMA(1,20)
###########################
alpha_list = c(0.1,0.3,0.5,0.7,0.9)
coverage_cal = c()
length_cal = c()
...
for(alpha in alpha_list){
  load(paste0("sparse_linear_5_AR1_MA20_alpha-", alpha,".RData"))
...
data <- data.frame(coverage = c(coverage_cal, coverage_cal_0, coverage_cal_mid, coverage_cal_full), alpha = rep(alpha_list, 4),
                   ...
                   Method = rep(c("QFCV(1)", "QFCV(0)", "QFCV(2)", "QFCV(5)"), each = length(coverage_cal)))
